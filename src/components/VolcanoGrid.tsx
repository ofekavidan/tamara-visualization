...
// Plotly לא עובד ב-SSR, נטען דינמית
...
/** CSV parser פשוט (הקבצים כאן לא מכילים פסיקים בטקסטים) */
    ...
    async function load(tp: Timepoint): Promise<Bucket> {
      try {
        ...
        for (const r of rows) {
          ...
          const padj = toNum(pick(r, ["padj","P.adj"]));
          let p = (pval ?? padj);
          if (p == null) continue;
          if (!(Number.isFinite(p))) continue;
          if (p <= 0) p = EPS;                // ← כאן התיקון המרכזי
        ...
        const base = {
          ...
          marker: { size: 6, color: "rgba(0,0,0,0.55)" }, // שחור למילוי ברירת מחדל
  ...
  function buildHighlight(tp: Timepoint) {
    ...
    if (colorMode === "significance") {
      const colors = b.x.map((x, i) => {
        ...
        if (passPadj && passFC) return "#ef4444"; // שניהם – אדום (כמו בדוגמא של תמרה)
  ...
  function VolcanoCard({ tp }: { tp: Timepoint }) {
    ...
    return (
      <div className="rounded-md border-2 border-[#2C5F7C] overflow-hidden bg-white">
        <Plot data={data} layout={cardLayout()} config={{ displayModeBar: false }} style={{ width: "100%", height: "100%" }} />
    ...
    <section className="w-full">
      ...
      <div className="mb-4 flex flex-wrap items-center gap-4">
        <div className="flex items-center gap-2">
          ...
          <label className="flex items-center gap-1">
            <input type="radio" name="cm" checked={colorMode==="significance"} onChange={()=>setColorMode("significance")} />
            significance
          </label>
          <label className="flex items-center gap-1">
            <input type="radio" name="cm" checked={colorMode==="name"} onChange={()=>setColorMode("name")} />
            RNA name
          </label>
          <label className="flex items-center gap-1">
            <input type="radio" name="cm" checked={colorMode==="family"} onChange={()=>setColorMode("family")} />
            tRF family
          </label>
        </div>
        ...
        {colorMode === "family" && rnaClass === "tRFs" && (
          <div className="flex items-center gap-2 flex-wrap">
            <input className="px-3 py-1 border rounded-md" placeholder="tRF_type"  value={family.tRF_type}
                   onChange={(e)=>setFamily(f=>({...f,tRF_type:e.target.value}))} />
            <input className="px-3 py-1 border rounded-md" placeholder="origin"     value={family.origin}
                   onChange={(e)=>setFamily(f=>({...f,origin:e.target.value}))} />
            <input className="px-3 py-1 border rounded-md" placeholder="aminoAcid" value={family.aminoAcid}
                   onChange={(e)=>setFamily(f=>({...f,aminoAcid:e.target.value}))} />
            <span className="text-xs text-slate-500">(מספיק גם חלק מהערך, למשל “nuclear”, “Ala”)</span>
          </div>
        )}
      </div>
      ...
      {!hasRealData && (
        <p className="text-xs text-center text-orange-600 mt-3">
          מציג כרגע נתוני placeholder (לא נמצאו קבצי DE תחת <code>/public/de/</code>).
        </p>
      )}
